...
- **Use only the studio font set.** For any design, document, or visual, use only the studio's installed fonts: Inter, IBM Plex Sans, IBM Plex Mono, Montserrat, Lato, Nunito, Playfair Display, Archivo Black, Manrope, Bitcount Prop Single, Chelsea Market, Libre Baskerville, Changa One, EB Garamond, Urbanist, League Script, Dancing Script, and Yesteryear. Use a different font only if the user provides one or asks for a specific one. See `document-design` for which font fits which role.
